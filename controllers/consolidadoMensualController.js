...
export const generarResumenMensual = async (req, res) => {
    try {
        if (req.user.rol !== "Fundador") {
            return res
                .status(403)
                .json({ message: "Solo el Fundador puede generar el consolidado mensual." });
        }
        ...
        const fechaInicioMes = moment
            .tz(`${anio}-${mesStr}-01`, TZ)
            .startOf("month")
            .toDate();

        const fechaFinMes = moment.tz(fechaInicioMes, TZ).endOf("month").toDate();

        // ✅ Ordenamos para que saldoInicial sea realmente el del primer día del mes con resumen
        ...
        const ingresosTotales = resumenes.reduce(
            (acc, r) => acc + (Number(r.ingresosTotales) || 0),
            0
        );
        const egresosTotales = resumenes.reduce(
            (acc, r) => acc + (Number(r.egresosTotales) || 0),
            0
        );
        ...
        return res.status(500).json({ message: "Error al generar consolidado mensual." });
    }
};
...
export const exportarResumenMensualPDF = async (req, res) => {
    try {
        ...
        doc.text(`Fecha de generación: ${moment().tz(TZ).format("DD/MM/YYYY HH:mm")}`);
        doc.moveDown(1.5);

        doc.fontSize(12).text(`Saldo inicial del mes: $${Number(consolidado.saldoInicial || 0).toLocaleString()}`);
        doc.text(`Ingresos del mes:      $${Number(consolidado.ingresosTotales || 0).toLocaleString()}`);
        doc.text(`Egresos del mes:       $${Number(consolidado.egresosTotales || 0).toLocaleString()}`);
        doc.text(`Saldo final:           $${Number(consolidado.saldoFinal || 0).toLocaleString()}`);
        doc.moveDown(1.5);
        ...
        return res.status(500).json({ message: "Error al generar PDF del consolidado mensual." });
    }
};
