    ...
    /**
     * 🟢 Crear una cita
     * Fundador / Profesional / Asistente pueden crear
     * Profesional siempre se asigna a sí mismo
     */
    export const crearCita = async (req, res) => {
        try {
            const { paciente, fecha, duracion, tipo, notas, profesional: profesionalId } = req.body;

            const organizacion = req.user.organizacion;

            // 🔹 Regla: si es profesional → la cita queda asignada SOLO a él.
            // Fundador y Asistente pueden asignar a cualquier profesional.
            const profesional =
                req.user.rol === "Profesional"
                    ? req.user._id
                    : profesionalId || req.user._id;
    ...
    export const editarCita = async (req, res) => {
        try {
            ...
            // 🔹 Profesional solo puede editar sus propias citas
            if (req.user.rol === "Profesional" && cita.profesional.toString() !== req.user._id.toString()) {
                return res.status(403).json({ message: "No tienes permisos para editar esta cita." });
            }
    ...
    export const cancelarCita = async (req, res) => {
        try {
            ...
            // 🔹 Profesional solo puede cancelar sus citas
            if (req.user.rol === "Profesional" && cita.profesional.toString() !== req.user._id.toString()) {
    ...
    export const exportarCitas = async (req, res) => {
        try {
            ...
            // 🔹 Profesional solo exporta sus citas
            if (req.user.rol === "Profesional") {
                filtro.profesional = req.user._id;
            }
